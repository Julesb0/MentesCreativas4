...
export const cylinderSurfaceArea = (radius: number, height: number) => {
	const r = sanitizeDimension(radius);
	const h = sanitizeDimension(height);
	return 2 * Math.PI * r * (h + r);
};

export const cylinderVolume = (radius: number, height: number) => {
	const r = sanitizeDimension(radius);
	const h = sanitizeDimension(height);
	return Math.PI * r * r * h;
};

export const cylinderMetrics = (radius: number, height: number): ShapeMetrics => ({
	surfaceArea: cylinderSurfaceArea(radius, height),
